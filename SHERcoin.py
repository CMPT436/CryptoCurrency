...
class Params:
    ...
    MAX_BOX_SIZE = 1000000  # bytes = 1MB
    COIN_MATURITY = 0
    MAX_FUTURE_BLOCK_TIME = (6 * 6 * 2)
    BELUSHIS_PER_COIN = int(100e6)
    TOTAL_COINS = 21_000_000
    MAX_VALUE_OF_COIN = BELUSHIS_PER_COIN * TOTAL_COINS
    DESIRED_SECOUNDS = 1 * 6
    DIFFICULTY_PERIOD_IN_SECS_TARGET = (6 * 6 * 1)
    DIFFICULTY_PERIOD_IN_BLOCKS = (DIFFICULTY_PERIOD_IN_SECS_TARGET / DESIRED_SECOUNDS)
    INITIAL_DIFFICULTY = 10
    HALVE_SUBSIDY_AFTER_BLOCKS_NUM = 200
...
class Transaction(NamedTuple):
    ...
    def validate_basics(self, as_coinbase=False):
        if (not self.txouts) or (not self.txins and not as_coinbase):
            raise TxnValidationError('Missing information for TXIN & TXOUT')
        if len(serialize(self)) > Params.MAX_BOX_SIZE:
            raise TxnValidationError('The transaction is to large')
        if sum(t.value for t in self.txouts) > Params.MAX_VALUE_OF_COIN:
            raise TxnValidationError('The money is maxed out, Ouch')

# the Block chain Class :
# blockid is the ID of the block. This is universial as only 1 block is created.
#The prev_hash is the
...
class Block(NamedTuple):
    ...
    # target. This also starts the chain to appropriate measures.
    ...
    def header(self, nonce=None) -> str:
        ...
        """
        return (
            f'{self.blockid}{self.prev_block_hash}{self.merkle_hash}'
...
genesis_block = Block(
        blockid=0, prev_block_hash=None,
        merkle_hash=(
...
def connect_block(block: Union[str, Block],
    ...
    if (not doing_reorg and reorg_if_necessary()) or \
        ...
        print(
            'block accepted ',
            'height={len(active_chain) - 1} txns={len(block.txns)}')
...
def get_next_work_required(prev_block_hash: str) -> int:
    """
    get the difficulty that the next block must solve
    """
    ...
    if actual_time_taken < Params.DIFFICULTY_PERIOD_IN_SECS_TARGET:
        # if the actual time was less than the target than increase difficulty
...
# returns a block that is sent to be mined
# It is called in mine forever.
# If the hash value meets the taget, it gets called in mine forever and tries to
# add it the block chain and waits for it to get validated
def assemble_and_solve_block(pay_coinbase_to_addr, txns=None):
    ...
    coinbase_txn = Transaction.create_coinbase(
                my_address, (get_block_subsidy()), len(active_chain))
    block = block._replace(txns=[coinbase_txn, *block.txns])
    block = block._replace(merkle_hash=get_merkle_root_of_txns(block.txns).val)

    if len(serialize(block)) > Params.MAX_BOX_SIZE:
        raise ValueError('txns specified create a block too large')
    return mine(block)


# used to tell thread that blockchain has been updated
# reward to miners for the approved transactions.
...
# try to create hash under the target value
# block is mined
# Mines from mempool.
def mine(block):
    start = time.time()
    ...
    while int(sha256d(block.header(nonce)), 16) >= target:
        ...
        if nonce % 10000 == 0 and mine_interrupt.is_set():
            logger.info('Sorry, Aleardy taken, Interepted()')
            mine_interrupt.clear()
            return None
...
def validate_signature_for_spend(txin, utxo: UnspentTxOut, txn):
    ...
    if pubkey_as_addr != utxo.to_address:
        raise TxUnlockError("Public key does not match the private key. ")
    ...
    try:
        ...
        logger.exception('validation of key failed')
...
def select_from_mempool(block: Block) -> Block:
    ...
    def check_block_size(b) -> bool:
        return len(serialize(block)) < Params.MAX_BOX_SIZE
